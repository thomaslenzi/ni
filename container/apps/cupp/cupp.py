...
def interactive(output_file=None):
    ...
    profile["wife"] = input("> Partner's name: ").lower()
    profile["wifen"] = input("> Partner's nickname: ").lower()
    ...
    generate_wordlist_from_profile(profile, output_file)  # generate the wordlist


def generate_wordlist_from_profile(profile, output_file):
    ...
    out_file = output_file if output_file else profile["name"] + ".txt"
    print_to_file(out_file, unique_list_finished)
...
def get_parser():
    ...
    parser.add_argument(
        "-o", "--output", type=str, help="File to output the generated wordlist to"
    )
